  ...
  >
    <div class="pr-8">
      <el-form
        ...
        :rules="formRules"
      >
        <el-form-item label="当前余额">{{ value }}条 </el-form-item>
        ...
        <el-form-item label="调整余额" prop="num">
          <el-input
            ...
            placeholder="请输入调整的条数"
            type="number"
            @input="numberValidate"
          />
        </el-form-item>
        <el-form-item label="调整后余额">
          {{ adjustmentMoney }} 条</el-form-item
        >
      </el-form>
    </div>
  </popup>
...
const adjustmentMoney = computed(() => {
  return (
    Number(props.value) + Number(formData.num) * (formData.action == 1 ? 1 : -1)
  );
});
