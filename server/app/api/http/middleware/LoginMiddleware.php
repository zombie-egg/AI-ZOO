...
class LoginMiddleware
{
    ...
    {
        // 自助机使用独立的设备代理令牌，不应被普通用户登录 token 拦截。
        // 控制器仍会再次校验同一令牌，避免把此分支变成匿名入口。
        $path = ltrim((string)$request->pathinfo(), '/');
        $expectedKioskToken = (string)env('kiosk.device_proxy_token', '');
        $actualKioskToken = (string)$request->header('X-Kiosk-Token', '');
        if (str_starts_with($path, 'kiosk/')
            && $expectedKioskToken !== ''
            && hash_equals($expectedKioskToken, $actualKioskToken)) {
            $request->userInfo = [];
            $request->userId = 0;
            return $next($request);
        }
        ...
        if (empty($token) && !$isNotNeedLogin) {
            ...
            return JsonService::fail('Login request failed', [], 0, 0);
        }
        ...
        if (empty($userInfo) && !$isNotNeedLogin) {
            ...
            return JsonService::fail('Login status is expired, retry again', [], BusinessErrorCodeEnum::USER_TOKEN_EXPIRED, 0);
        }
        ...
        if ($userInfo) {
            ...
            if (time() > ($userInfo['expire_time'] - $beExpireDuration)) {
                ...
                //续期失败（数据表被删除导致）
                if (empty($result)) {
                    return JsonService::fail('Login status is expired', [], BusinessErrorCodeEnum::USER_TOKEN_EXPIRED);
                }
            }
        }
