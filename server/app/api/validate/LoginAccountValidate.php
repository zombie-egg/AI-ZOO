...
class LoginAccountValidate extends BaseValidate
{
    ...
    protected $rule = [
        ...
        'scene' => 'require|in:' . LoginEnum::MOBILE_LOGIN . ',' . LoginEnum::ACCOUNT_LOGIN . '|checkConfig',
        'account' => 'require',
    ];
    ...
    {
        // 手机号密码登录
        if (LoginEnum::ACCOUNT_LOGIN == $scene) {
    ...
    public function checkPassword($password, $other, $data)
    {
        ...
        $where = [];
        if ($data['scene'] == LoginEnum::ACCOUNT_LOGIN) {
            // 手机号密码登录
    ...
    public function checkCode($code, $rule, $data)
    {
        ...
        JsonService::throw('Wrong verification code');
    }
}
