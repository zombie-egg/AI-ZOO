...
//            file_exists($realPath) && unlink($realPath);
            ...
            // ImageForge 私有保存顾客原图；前端头像继续用 PHP 已保存的本地图，避免暴露私有目录。
            $imageUrl = LunaDrawService::isImageForgeEnabled()
                ? $res['localFileRes']['url']
                : LunaDrawService::getOssBaseUrl() . '/' . $res['filePath'];
            $image = LunaDrawService::isPhase1StubEnabled() ? null : new ImageCropService($imageUrl);
            ...
            foreach ($faceList as $face) {
                $newRow = [
                    ...
                    'image_url' => $imageUrl,
                    'face_url' => $image
                        ? $image->cropByRatio($face['boundingBoxLeft'], $face['boundingBoxTop'], $face['boundingBoxWidth'], $face['boundingBoxHeight'])
                        : $imageUrl,
