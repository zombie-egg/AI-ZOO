...
use app\common\service\Phase1OfflineAcceptanceService;
...
class LoginController extends BaseApiController
{
     ...
     * Phase 1 本机离线验收登录。
     *
     * 该入口不冒充微信 code2session；只有忽略文件中的显式开关开启，且 Host 为
     * localhost/127.0.0.1 时才存在。生产默认返回 404。
     */
    public function phase1OfflineLogin()
    {
        if (!Phase1OfflineAcceptanceService::requestAllowed($this->request)) {
            return json(['code' => 0, 'show' => 0, 'msg' => 'Not Found', 'data' => null], 404);
        }
        $res = LoginLogic::phase1OfflineLogin();
        if (false === $res) {
            return $this->fail(LoginLogic::getError());
        }
        return $this->success('Phase 1 offline acceptance login', $res);
    }
