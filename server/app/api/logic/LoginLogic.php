...
class LoginLogic extends BaseLogic
{
    ...
    /**
     * 仅供官方 Luna/微信网关均不可用时的本机 Phase 1 离线验收。
     * 用户创建、微信终端 token、登录信息和后续鉴权仍走正式业务代码。
     */
    public static function phase1OfflineLogin()
    {
        Db::startTrans();
        try {
            $userServer = new WechatUserService([
                'openid' => 'phase1-offline-acceptance-user',
                'unionid' => '',
                'nickname' => 'Phase 1 合成测试用户',
            ], UserTerminalEnum::WECHAT_MMP);
            $userInfo = $userServer->getResopnseByUserInfo()->authUserLogin()->getUserInfo();

            $fixtureUrl = LunaDrawService::getOssBaseUrl() . '/synthetic-input.png';
            $avatar = DigitalAvatar::where([
                'user_id' => $userInfo['id'],
                'up_file_id' => 910001,
                'up_face_id' => 910101,
            ])->findOrEmpty();
            if ($avatar->isEmpty()) {
                DigitalAvatar::create([
                    'user_id' => $userInfo['id'],
                    'image_url' => $fixtureUrl,
                    'face_url' => $fixtureUrl,
                    'file_id' => 0,
                    'up_file_id' => 910001,
                    'up_face_id' => 910101,
