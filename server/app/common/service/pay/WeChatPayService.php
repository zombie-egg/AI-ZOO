...
class WeChatPayService extends BasePayService
{
    ...
    public function __construct($terminal, $userId = null)
    {
        ...
        if (empty($this->config['mch_id'])) {
            LogUtils::log('微信支付配置不正确', ['config' => $this->config]);
            throw new BaseException("微信支付配置不正确，请检查配置");
        }
        $this->app = new Application($this->config);
        if ($userId !== null) {
            $auth_terminal = $terminal == UserTerminalEnum::WECHAT_OA ? [UserTerminalEnum::PC, UserTerminalEnum::WECHAT_OA] : $terminal;
            $this->auth = UserAuth::where(['user_id' => $userId, 'terminal' => $auth_terminal])->findOrEmpty();
        }
    }
            ...
            if ($message['trade_state'] === 'SUCCESS') {
                ...
                switch ($attach) {
                    case 'recharge':
                        ...
                        break;
                    case 'member':
                        $order = MemberOrder::where(['sn' => $message['out_trade_no']])->findOrEmpty();
                        if ($order->isEmpty() || $order->pay_status == PayEnum::ISPAID) {
                            return true;
                        }
                        ...
                        break;
                    case 'unlock':
                        $order = UnlockOrder::where(['sn' => $message['out_trade_no']])->findOrEmpty();
                        if ($order->isEmpty() || $order->pay_status == PayEnum::ISPAID) {
                            return true;
                        }
                        $paidCents = (int)($message['amount']['payer_total'] ?? $message['amount']['total'] ?? -1);
                        $currency = (string)($message['amount']['currency'] ?? 'CNY');
                        $expectedCents = (int)round(((float)$order->order_amount) * 100);
                        if ($paidCents !== $expectedCents || $currency !== 'CNY') {
                            throw new \RuntimeException('解锁订单支付金额或币种不一致');
                        }
                        PayNotifyLogic::handle('unlock', $message['out_trade_no'], $extra);
                        break;
                }
            }
            return true;
