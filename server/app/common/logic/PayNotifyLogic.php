...
class PayNotifyLogic extends BaseLogic
{
    ...
    {
        $order = UnlockOrder::where('sn', $orderSn)->lock(true)->findOrEmpty();
        if ($order->isEmpty()) {
            throw new \RuntimeException('解锁订单不存在');
        }
        ...
        $kiosk = KioskOrder::where('id', $order->kiosk_order_id)->lock(true)->findOrEmpty();
        if ($kiosk->isEmpty()) {
