...
//            }
            ...
            foreach ($pay_way as $k => &$item) {
                ...
                if ($item['pay_way'] == PayEnum::DOUYIN_PAY) {
                    $item['extra'] = '抖音支付';
                }
...
//                'order_amount' => $order['order_amount'],
        ...
        try {
            ...
            switch ($params['from']) {
                ...
                case 'member':
                    ...
                    break;
                case 'unlock':
                    $order = UnlockOrder::where(['id' => $params['order_id']])->findOrEmpty();
                    $payTime = empty($order['pay_time']) ? '' : date('Y-m-d H:i:s', $order['pay_time']);
                    $orderInfo = [
                        'order_id' => $order['id'],
                        'order_sn' => $order['sn'],
        ...
        try {
            switch ($params['from']) {
                case 'recharge':
                    ...
                    if ($order->isEmpty()) {
                        throw new \Exception('Recharge not exist');
                    }
                    break;
                case 'member':
                    ...
                    break;
                case 'unlock':
                    $order = UnlockOrder::findOrEmpty($params['order_id']);
                    if ($order->isEmpty()) {
                        throw new \Exception('解锁订单不存在');
                    }
                    break;
            }
    ...
    public static function pay($payWay, $from, $order, $terminal, $redirectUrl)
    {
        ...
        // 免费无需支付，直接发货
        $isFree = $from == 'recharge' && floatval($order['order_amount']) == 0;
        if ($isFree) {
            $payWay = PayEnum::BALANCE_PAY;
        }

        //更新支付方式
        $toUpdate = ['pay_way' => $payWay, 'pay_sn' => $paySn, 'is_free' => $isFree ? 1 : 0];
        switch ($from) {
            case 'recharge':
                RechargeOrder::update($toUpdate, ['id' => $order['id']]
                );
                break;
            case 'member':
                unset($toUpdate['is_free']);
                MemberOrder::update($toUpdate, ['id' => $order['id']]
                );
                break;
            case 'unlock':
                unset($toUpdate['is_free']);
                UnlockOrder::update($toUpdate, ['id' => $order['id']]);
                break;
        }
