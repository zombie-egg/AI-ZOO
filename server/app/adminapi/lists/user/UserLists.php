...
class UserLists extends BaseAdminDataLists implements ListsExcelInterface
{
    ...
    {
        $allowSearch = ['keyword', 'channel', 'create_time_start', 'create_time_end','is_member', 'provider'];
...
//            ->field($field)
        ...
        foreach ($lists as &$item) {
            ...
            $item['member_desc'] = $item['member_info']['member_name'] ?? '';
            $item['member_end_time_desc'] = $item['member_info']['member_end_time'] ?? '';
            $item['is_end'] = $item['member_info']['is_end'] ?? '';
        }
