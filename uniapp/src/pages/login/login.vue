<template>
  <view class="content">
    ...
    <!-- #endif -->
    <button
      v-if="phase1OfflineAcceptance"
      class="main-button offline-btn"
      :class="{ 'main-button-disabled': !agreeTerms }"
      :style="$getMediumFontWeight()"
      @tap="$debounceClick(requestPhase1OfflineLogin)()"
    >
      Phase 1 离线验收登录（非微信）
    </button>
...
<script>
import { mnpLogin, phase1OfflineLogin } from "../../api/user";
...
export default {
  data() {
    return {
      ...
      agreementContent: "",
      agreementTitle: "",
      phase1OfflineAcceptance:
        import.meta.env.VITE_PHASE1_OFFLINE_ACCEPTANCE === "true",
      maskImgSrc:
  ...
  methods: {
    ...
    requestPhase1OfflineLogin() {
      if (!this.agreeTerms) return;
      phase1OfflineLogin().then(this.onLoginSuccess).catch((err) => {
        console.log(err);
      });
    },
  },
